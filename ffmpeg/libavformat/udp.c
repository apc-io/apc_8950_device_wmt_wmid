...
#define DBG(fmt, args...)   printf("%s: " fmt, __FUNCTION__ , ## args)
...
#else
{
    	struct sockaddr_in si= *(struct sockaddr_in *)&my_addr;
    	 si.sin_family = AF_INET;
 	 si.sin_port = htons (port);
    	 si.sin_addr.s_addr = htonl(INADDR_ANY);

    	 DBG(" bind family  %d sin_port 0x%08x \n", si.sin_family, si.sin_port);
	 if (bind_ret < 0 && bind(udp_fd,(struct sockaddr *)&si, len) < 0)
	{
	    DBG("normal bind  fail\n");
	    goto fail;
	}else{
	    DBG("normal bind  OK udp_fd 0x%08x \n",udp_fd);
	}
