...
static const StreamType MISC_types[] = {
    { 0x81, AVMEDIA_TYPE_AUDIO,   CODEC_ID_AC3 },
	{ 0x83, AVMEDIA_TYPE_AUDIO,   CODEC_ID_PCM_S16BE },
                                  ...
                                  uint32_t stream_type, uint32_t prog_reg_desc)
{
	...
	// Stream_type in MISC_types, Define for WFD
    if (pes->stream_type == 0x83 && st->codec->codec_id == CODEC_ID_PCM_S16BE){
    ...
    while (buf_size > 0) {
        ...
        case MPEGTS_PESHEADER:
            ...
            if (pes->data_index == PES_HEADER_SIZE) {
				...
				// Stream_type in MISC_types, Define for WFD
				if (pes->stream_type == 0x83 && pes->st->codec->codec_id == CODEC_ID_PCM_S16BE)
					pes->pes_header_size += 4; // 4 is private header including channel and sample rate
        ...
        case MPEGTS_PESHEADER_FILL:
            ...
            if (pes->data_index == pes->pes_header_size) {
				...
				if (pes->stream_type == 0x83 && pes->st->codec->codec_id == CODEC_ID_PCM_S16BE){
					r += 2;
					if(*r == 0xa0 /* sub_stream_id */ && *(r+1) == 0x6 /* number_of_frame_header */){
						int sf, ch, bps;
						// audio_emphasis_flag = *(r+2) & 0x1; // 1= emphasis on
						bps = *(r+3) >> 6;
						sf = *(r+3) >> 3;
						ch = *(r+3) & 0x7;
						if(!(bps & 0x3))
							pes->st->codec->bits_per_coded_sample = 16;
						if((sf & 0x7) == 1)
							pes->st->codec->sample_rate = 44100;
						else if((sf & 0x7) == 2)
							pes->st->codec->sample_rate = 48000;
						if(!ch)
							pes->st->codec->channels = 1;
						else if(ch == 1)
                              ...
                              AVPacket *pkt)
{
	...
	// error handling: mpegts_read_header() may not parse Program Specific Information
	if(!ret && (!nb_prg || !ts->nb_prg)){
		av_log(s, AV_LOG_WARNING, "No PSI %d/%d in header. Retry!!\n",nb_prg,ts->nb_prg);
		//return AVERROR(EAGAIN);
	}
