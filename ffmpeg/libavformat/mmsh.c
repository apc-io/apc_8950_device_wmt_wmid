...
static ChunkType get_chunk_header(MMSHContext *mmsh, int *len)
{
    ...
    if (res != CHUNK_HEADER_LENGTH) {
        av_log(NULL, AV_LOG_ERROR, "[MMS-H] Read data packet header failed!\n");
        return AVERROR(EIO);
    }
    ...
    case CHUNK_TYPE_DATA:
        ...
        break;
    default:
        av_log(NULL, AV_LOG_ERROR, "[MMS-H] Strange chunk type %d\n", chunk_type);
        return AVERROR_INVALIDDATA;
    }

    res = url_read_complete(mms->mms_hd, ext_header, ext_header_len);
    if (res != ext_header_len) {
        av_log(NULL, AV_LOG_ERROR, "[MMS-H] Read ext header failed!\n");
        return AVERROR(EIO);
    }
...
static int read_data_packet(MMSHContext *mmsh, const int len)
{
    ...
    if (len > sizeof(mms->in_buffer)) {
        av_log(NULL, AV_LOG_ERROR,
               "[MMS-H] Data packet length %d exceeds the in_buffer size %zu\n",
               len, sizeof(mms->in_buffer));
        return AVERROR(EIO);
    }
    res = url_read_complete(mms->mms_hd, mms->in_buffer, len);
    // av_log(NULL, AV_LOG_DEBUG, "[MMS-H] Data packet len = %d\n", len);
    if (res != len) {
        av_log(NULL, AV_LOG_ERROR, "[MMS-H] Read data packet failed!\n");
        return AVERROR(EIO);
    }
    if (len > mms->asf_packet_len) {
        av_log(NULL, AV_LOG_ERROR,
               "[MMS-H] Chunk length %d exceed packet length %d\n",len, mms->asf_packet_len);
        return AVERROR_INVALIDDATA;
    } else {
...
static int get_http_header_data(MMSHContext *mmsh)
{
    ...
    for (;;) {
        ...
        if (res < 0) {
            ...
            if (!mms->header_parsed) {
                if (mms->asf_header) {
                    if (len != mms->asf_header_size) {
                        ...
                        av_log(NULL, AV_LOG_DEBUG, "[MMS-H] Header len changed from %d to %d\n",
                                mms->asf_header_size, len);
            ...
            if (len > mms->asf_header_size) {
                av_log(NULL, AV_LOG_ERROR,
                       "[MMS-H] Asf header packet len = %d exceed the asf header buf size %d\n",
            ...
            if (res != len) {
                av_log(NULL, AV_LOG_ERROR,
                       "[MMS-H] Recv asf header data len %d != expected len %d\n", res, len);
                return AVERROR(EIO);
            }
            mms->asf_header_size = len;
            if (!mms->header_parsed) {
            ...
            if (len) {
                if (len > sizeof(mms->in_buffer)) {
                    av_log(NULL, AV_LOG_ERROR,
                           "[MMS-H] Other packet len = %d exceed the in_buffer size %zu\n",
                           len, sizeof(mms->in_buffer));
                    return AVERROR(EIO);
                }
                res = url_read_complete(mms->mms_hd, mms->in_buffer, len);
                if (res != len) {
                    av_log(NULL, AV_LOG_ERROR, "[MMS-H] Read other chunk type data failed!\n");
                    return AVERROR(EIO);
                } else {
                    av_log(NULL, AV_LOG_DEBUG, "[MMS-H] Skip chunk type %d \n", chunk_type);
                    continue;
                }
            }
        }
...
static int mmsh_open_internal(URLContext *h, const char *uri, int flags, int timestamp, int64_t pos)
{
	...
	nflags = URL_RDONLY | (flags & (URL_HTTP_NORANGE | URL_HTTP_HANDSHAKE));
    if(url_alloc(&mms->mms_hd, httpname, nflags) < 0) {
        av_log(NULL, AV_LOG_ERROR, "[MMS-H] Url allocation failed! (%s)\n",httpname);
        return AVERROR(EIO);
    }
    ...
    if (err) {
        av_log(NULL, AV_LOG_ERROR, "[MMS-H] Url connection failed!\n");
        goto fail;
    }
    err = get_http_header_data(mmsh);
    if (err) {
        av_log(NULL, AV_LOG_ERROR, "[MMS-H] Get http header data failed!\n");
        goto fail;
    }
    ...
    if ((err = url_alloc(&mms->mms_hd, httpname, nflags)) < 0) {
        av_log(NULL, AV_LOG_ERROR, "[MMS-H] Url allocation failed! (%s)\n",httpname);
        goto fail;
    }
    stream_selection = av_mallocz(mms->stream_num * 19 + 1);
    if (!stream_selection){
        av_log(NULL, AV_LOG_ERROR, "[MMS-H] stream selection allcation failed!\n");
        return AVERROR(ENOMEM);
    }
    ...
    if (err < 0) {
        av_log(NULL, AV_LOG_ERROR, "[MMS-H] Build play request failed!\n");
        goto fail;
    }
    ...
    av_log(NULL, AV_LOG_INFO, "[MMS-H] Connection successfully open\n");
    return 0;
fail:
    av_freep(&stream_selection);
    mmsh_close(h);
    av_log(NULL, AV_LOG_INFO, "[MMS-H] Connection failed with error %d\n", err);
    return err;
}
...
static int handle_chunk_type(MMSHContext *mmsh)
{
    ...
    case CHUNK_TYPE_END:
        mmsh->chunk_seq = 0;
        av_log(NULL, AV_LOG_ERROR, "[MMS-H] Stream ended!\n");
        return AVERROR(EIO);
    case CHUNK_TYPE_STREAM_CHANGE:
        mms->header_parsed = 0;
        if (res = get_http_header_data(mmsh)) {
            av_log(NULL, AV_LOG_ERROR,"[MMS-H] Stream changed! Failed to get new header!\n");
            return res;
        }
        break;
    case CHUNK_TYPE_DATA:
        return read_data_packet(mmsh, len);
    default:
        av_log(NULL, AV_LOG_ERROR, "[MMS-H] Recv other type packet %d\n", chunk_type);
        return AVERROR_INVALIDDATA;
    }
    return 0;
}
