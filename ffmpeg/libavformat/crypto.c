...
//    {"iv",  "AES decryption initialization vector", OFFSET(iv),  AV_OPT_TYPE_BINARY, .flags = D },
    {"key", "AES decryption key", OFFSET(key), FF_OPT_TYPE_BINARY, 0, 0, 0, D},
    {"iv",  "AES decryption initialization vector", OFFSET(iv), FF_OPT_TYPE_BINARY, 0, 0, 0, D},
    { NULL }
};
...
static const AVOption options[] = {
	{"cs", "it", OFFSET(chunksize), FF_OPT_TYPE_INT64, 0, -1, 0 }, /* Default to 0, for chunked POSTs */
	{NULL}
};
...
void ff_crypto_set_key(URLContext *h, uint8_t *headers);
void ff_crypto_set_iv(URLContext *h, uint8_t *headers);
...
void ff_crypto_set_key(URLContext *h, uint8_t *key)
{
    CryptoContext *c = h->priv_data;
	set_string_binary(key,&c->key);
}

void ff_crypto_set_iv(URLContext *h, uint8_t *iv)
{
    CryptoContext *c = h->priv_data;
	set_string_binary(iv,&c->key);
}
...
static int crypto_open(URLContext *h, const char *uri, int flags)
{
	...
	if ((ret = url_open(&c->hd, nested_url, URL_RDONLY)) < 0) {
...
retry:
    ...
    while (c->indata - c->indata_used < 2*BLOCKSIZE) {
        ...
        int n = url_read(c->hd, c->inbuffer + c->indata,
                         sizeof(c->inbuffer) - c->indata);
        if (n <= 0) {
            c->eof = 1;
            break;
