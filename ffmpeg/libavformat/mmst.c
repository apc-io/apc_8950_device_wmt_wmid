...
// write_result= ffurl_write(mms->mms_hd, mms->out_buffer, exact_length);
    ...
    if(write_result != exact_length) {
        av_log(NULL, AV_LOG_ERROR,
               "[MMS-T] Failed to write data of length %d: %d (%s)\n",
...
{
    MMSContext *mms = &mmst->mms;
    av_log(NULL, AV_LOG_DEBUG, "[MMS-T] Stream changing!\n");

    // 40 is the packet header size, 7 is the prefix size.
    mmst->header_packet_id= AV_RL32(mms->in_buffer + 40 + 7);
    av_log(NULL, AV_LOG_DEBUG, "[MMS-T] Changed header prefix to 0x%x", mmst->header_packet_id);
}
...
static MMSSCPacketType get_tcp_server_response(MMSTContext *mmst)
{
    ...
    for(;;) {
        ...
        if (read_result != 8) {
            if(read_result < 0) {
                av_log(NULL, AV_LOG_ERROR,
                       "[MMS-T] Error reading packet header: %d (%s)\n",
                       read_result, strerror(read_result));
                packet_type = SC_PKT_CANCEL;
            } else {
                av_log(NULL, AV_LOG_ERROR,
                       "[MMS-T] The server closed the connection\n");
        ...
        if(AV_RL32(mms->in_buffer + 4)==0xb00bface) {
            ...
            if(read_result != 4) {
                av_log(NULL, AV_LOG_ERROR,
                       "[MMS-T] Reading command packet length failed: %d (%s)\n",
                       read_result,
            ...
            av_log(NULL, AV_LOG_DEBUG, "[MMS-T] Length remaining is %d\n", length_remaining);
            // read the rest of the packet.
            if (length_remaining < 0
                || length_remaining > sizeof(mms->in_buffer) - 12) {
                av_log(NULL, AV_LOG_ERROR,
                       "[MMS-T] Incoming packet length %d exceeds bufsize %zu\n",
                       length_remaining, sizeof(mms->in_buffer) - 12);
            ...
            if (read_result != length_remaining) {
                av_log(NULL, AV_LOG_ERROR,
                       "[MMS-T] Reading pkt data (length=%d) failed: %d (%s)\n",
            ...
            if (read_result >= 44 && (hr = AV_RL32(mms->in_buffer + 40))) {
                av_log(NULL, AV_LOG_ERROR,
                       "[MMS-T] Server sent a message with packet type 0x%x and error status code 0x%08x\n", packet_type, hr);
            ...
            if (length_remaining < 0
                ...
                av_log(NULL, AV_LOG_ERROR,
                       "[MMS-T] Data length %d is invalid or too large (max=%zu)\n",
            ...
            if(read_result != length_remaining) {
                av_log(NULL, AV_LOG_ERROR,
                       "[MMS-T] Failed to read packet data of size %d: %d (%s)\n",
                       length_remaining, read_result,
                       read_result < 0 ? strerror(read_result) :
                           "The server closed the connection");
                return read_result < 0 ? read_result : AVERROR(EIO);
            ...
            if(packet_id_type == mmst->header_packet_id) {
                ...
                av_log(NULL, AV_LOG_DEBUG, "[MMS-T] packet id type %d is old.", packet_id_type);
                continue;
            }
        }
                              ...
                              const MMSSCPacketType expect_type)
{
    ...
    if(send_fun) {
        ...
        if (ret < 0) {
            av_log(NULL, AV_LOG_ERROR, "[MMS-T] Send Packet error before expecting recv packet %d\n", expect_type);
            return ret;
        }
    }
    ...
    if ((type = get_tcp_server_response(mmst)) != expect_type) {
        av_log(NULL, AV_LOG_ERROR,
               "[MMS-T] Corrupt stream (unexpected packet type 0x%x, expected 0x%x)\n",
               type, expect_type);
        return AVERROR_INVALIDDATA;
    } else {
        return 0;
...
static int mms_open(URLContext *h, const char *uri, int flags)
{
    ...
    if((mmst->incoming_flags != 0X08) && (mmst->incoming_flags != 0X0C)) {
        av_log(NULL, AV_LOG_ERROR,
               "[MMS-T] The server does not support MMST (try MMSH or RTSP)\n");
        err = AVERROR(EINVAL);
        goto fail;
    }
    err = ff_mms_asf_header_parser(mms);
    if (err) {
        av_log(NULL, AV_LOG_ERROR, "[MMS-T] asf header parsed failed!\n");
        goto fail;
    }
    mms->header_parsed = 1;
    ...
    av_log(NULL, AV_LOG_INFO, "[MMS-T] Leaving open (success)\n");
    return 0;
fail:
    mms_close(h);
    av_log(NULL, AV_LOG_INFO, "[MMS-T] Leaving open (failure: %d)\n", err);
    return err;
}
    ...
    do {
        if(mms->asf_header_read_size < mms->asf_header_size) {
            ...
            if (err == 0) {
                if(mms->remaining_in_len>mms->asf_packet_len) {
                    av_log(NULL, AV_LOG_ERROR,
                           "[MMS-T] Incoming pktlen %d is larger than ASF pktsize %d\n",
                    ...
                    result = ff_mms_read_data(mms, buf, size);
                    if (result == 0) {
                        av_log(NULL, AV_LOG_ERROR, "[MMS-T] read asf media paket size is zero!\n");
                        break;
                    }
                }
            } else {
                av_log(NULL, AV_LOG_ERROR, "[MMS-T] read packet error!\n");
                break;
            }
        }
