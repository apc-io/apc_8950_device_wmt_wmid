...
static int open_input(struct variant *var)
{
    ...
    if (seg->key_type == KEY_NONE) {
        ...
        return url_open(&var->input, seg->url, URL_RDONLY);
        ...
        if (strcmp(seg->key, var->key_url)) {
			...
			if (url_open(&uc, seg->key, URL_RDONLY) == 0) {
                // if (ffurl_read_complete(uc, var->key, sizeof(var->key))
                //    != sizeof(var->key)) {
				if (url_read_complete(uc, var->key, sizeof(var->key))
					!= sizeof(var->key)) {
        ...
        if ((ret = url_alloc(&var->input, url, URL_RDONLY)) < 0)
            return ret;
        // av_opt_set(var->input->priv_data, "key", key, 0);
        // av_opt_set(var->input->priv_data, "iv", iv, 0);
		ff_crypto_set_key(var->input,key);
		ff_crypto_set_iv(var->input,iv);
        // if ((ret = ffurl_connect(var->input, NULL)) < 0) {
        //     ffurl_close(var->input);
		if ((ret = url_connect(var->input)) < 0) {
			url_close(var->input);
            var->input = NULL;
            return ret;
        }
...
static int applehttp_read_header(AVFormatContext *s, AVFormatParameters *ap)
{
    ...
    for (i = 0; i < c->n_variants; i++) {
        ...
        //                   read_data, NULL, NULL);
        init_put_byte(&v->pb, v->read_buffer, INITIAL_BUFFER_SIZE, 0, v,
                          read_data, NULL, NULL);
        // v->pb.seekable = 0;
        // ret = av_probe_input_buffer(&v->pb, &in_fmt, v->segments[0]->url,
        //                             NULL, 0, 0);
        v->pb.is_streamed = 1;
        ret = ff_probe_input_buffer(&v->pb, &in_fmt, v->segments[0]->url,
                                    NULL, 0, 0);
        if (ret < 0)
            goto fail;
        v->ctx->pb       = &v->pb;
        // ret = avformat_open_input(&v->ctx, v->segments[0]->url, in_fmt, NULL);
        ret = av_open_input_file(&v->ctx, v->segments[0]->url, in_fmt, 0, NULL);
        if (ret < 0)
            goto fail;
