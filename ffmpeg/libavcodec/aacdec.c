            ...
            if (band_type[idx] == ZERO_BT) {
                ...
                for (; i < run_end; i++, idx++) {
                    ...
                    if (offset[2] != clipped_offset) {
                        av_log(ac->avctx, AV_LOG_ERROR,"Intensity stereo "
                ...
                for (; i < run_end; i++, idx++) {
                    ...
                    if (offset[1] != clipped_offset) {
                        av_log(ac->avctx, AV_LOG_ERROR,"Noise gain clipped "
                                   ...
                                   float *in, IndividualChannelStream *ics)
{
    ...
    if (ics->window_sequence[0] != LONG_STOP_SEQUENCE) {
        ac->dsp.vector_fmul(in, in, 1024);
    } else {
        memset(in, 0, 448 * sizeof(float));
        ac->dsp.vector_fmul(in + 448, in + 448, 128);
...
AVCodec aac_decoder = {
    ...
    .sample_fmts = (const enum SampleFormat[]) {
        SAMPLE_FMT_FLT, SAMPLE_FMT_S16, SAMPLE_FMT_NONE
    },
