...
// Jason Modified cause there is an issue when num is bigger than 32
// #   define SKIP_BITS(name, gb, num)\
//        {\
//            SKIP_CACHE(name, gb, num)\
//            SKIP_COUNTER(name, gb, num)\
...
#   define SKIP_BITS(name, gb, num)\
        {\
			int ext = num, ws = 32;\
			if(ext > 32){ \
				SKIP_CACHE(name, gb, ws)\
				SKIP_COUNTER(name, gb, ws)\
				UPDATE_CACHE(name, gb)\
				av_log( NULL, AV_LOG_DEBUG, "BUG: GB overskip(%d) > 32! Force breakdown.\n", ext);\
				ext -= 32;\
				if(ext > 32) \
					av_log( NULL, AV_LOG_ERROR, "BUG: GB overskip %d!\n", ext + 32);\
			}\
            SKIP_CACHE(name, gb, ext)\
            SKIP_COUNTER(name, gb, ext)\
        }\
