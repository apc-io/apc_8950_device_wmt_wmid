...
static av_cold int decode_init(AVCodecContext *avctx)
{
	...
	if (avctx->channels > 0 && avctx->request_channels < avctx->channels &&
		...
		//av_log(avctx, AV_LOG_INFO, "*** WMAPRO Downmixed ***\n");
		s->temp_buf = av_mallocz(MAX_FRAMESIZE / 2);
	}	
                         ...
                         void *data, int *data_size, AVPacket* avpkt)
{
    ...
    *data_size = (int8_t *)s->samples - (int8_t *)data;
	...
	/* Doing down-mix process if necessary */
	if ((s->num_channels == 6) && (*data_size) && 
		((MAX_FRAMESIZE / 2) >= (*data_size / 3))) {
		
		*data_size /= 3;
		dec_sample = s->temp_buf;

		for (i = 0; i < ((*data_size) * 3 / sizeof(ch_data)); ++i) {
			ch_data = *((channel_data_t *)data + i);
			
			//Lt = L + -3dB*C + -3dB*(-Ls -Rs)
			*dec_sample = ch_data.L + (M_SQRT1_2 * ch_data.C) + 
							(M_SQRT1_2 * ((-1 * ch_data.LS) - ch_data.RS));
			dec_sample++;
			
			//Rt = R + -3dB*C + -3dB*(Ls + Rs)
			*dec_sample = ch_data.R + (M_SQRT1_2 * ch_data.C) + 
							(M_SQRT1_2 * (ch_data.LS + ch_data.RS));
			dec_sample++;
		}

		memcpy(data, s->temp_buf, *data_size);		
	}	
